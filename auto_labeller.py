...
from preprocess import load_df, save_df, get_text_embeddings
...
possible_labels = possible_labels_2
...
L = label_emb_df.values

# Create a list of distance matrices (one per feature_columns)
dist_mats = []
for col in feature_columns:
    emb_df, _ = get_text_embeddings(df, col, embedding_type="ST")
    S_col = cosine_similarity(emb_df.values, L)
    D_col = 1.0 - S_col
...
if len(dist_mats) == 1:
    ...
    missing = [col for col in feature_columns if col not in feature_weights]
    if missing:
        raise KeyError(f"Missing weights for columns: {missing}")
    w = np.array([feature_weights[col] for col in feature_columns], dtype=float)
    D_stack = np.stack(dist_mats, axis=0)
    D_avg = np.tensordot(w, D_stack, axes=(0, 0))

# Calculate label with the smallest distance
...
# Calculate top k labels
k = min(3, len(labels))
topk_idx = np.argsort(D_avg, axis=1)[:, :k]
...
save_df(out, output_path, ENFORCED_TEXT_COLS)
